...
app.get('/api/file', async (req, res) => {
    try {
        ...
        if (['.pdf', '.jpg', '.jpeg', '.png', '.gif', '.mp4', '.mp3'].includes(fileExtension)) {
...
app.post('/api/edit', express.json(), async (req, res) => {
    try {
        ...
        const fileExtension = path.extname(filePath).toLowerCase();
        const editableExtensions = ['.txt', '.md', '.json', '.js', '.css', '.html', '.xml'];
        ...
        res.status(500).json({ error: 'Failed to edit file', message: error.message });
    }
});
...
app.post('/api/delete', express.json(), async (req, res) => {
    try {
        ...
        if (fullPath === path.join(__dirname, 'LibraryFolder')) {
            return res.status(403).json({ error: 'Cannot delete LibraryFolder root' });
        }
        ...
        async function getFolders(dir, basePath = '') {
            ...
            for (const item of items) {
                if (item.isDirectory()) {
                    ...
                    folders.push({
                        name: item.name,
                        path: folderPath === 'LibraryFolder' ? 'LibraryFolder' : `LibraryFolder/${folderPath}`
                    });
