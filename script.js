...
function updateBreadcrumb(path) {
    ...
    if (!path) {
        breadcrumb.innerHTML = '<span onclick="navigateToPath(\'\')">Library</span>';
        return;
    }
...
function updateBackButton() {
    const backBtn = document.getElementById('backBtn');
    backBtn.style.display = AppState.navigationHistory.length > 0 ? 'block' : 'none';
}
...
async function saveFile() {
    ...
    try {
        ...
        const fileContent = document.getElementById('fileContent');
        const fileExtension = AppState.currentFilePath.split('.').pop().toLowerCase();
...
async function handleUpload(event) {
    ...
    if (!fileInput.files || fileInput.files.length === 0) {
        uploadStatus.innerHTML = '<div class="error-message">Please select a file to upload.</div>';
        return;
    }
...
async function loadFoldersForUpload() {
    try {
        ...
        data.folders.forEach(folder => {
            ...
            option.textContent = folder.path.replace('LibraryFolder/', '') || 'Library Root';
